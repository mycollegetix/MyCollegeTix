...
function parseEventLines(eventLines, date, sourceFile) {
  try {
    ...
    const sportMatch = cleanLine.match(
      /^(Soccer|Football|Volleyball|Field Hockey|Basketball|Ice Hockey|Hockey|Baseball|Softball|Tennis|Golf|Track|Cross Country|Wrestling|Gymnastics|Swimming|Diving)/i
    );
    if (!sportMatch) {
    ...
    const event = {
      ...
      status: "scraped",
      source: "parsed",
      external_id: generateExternalId(sport, date, opponent, isHomeGame),
...
async function checkForDuplicates(event) {
  try {
    // Use external_id for more reliable duplicate detection
    ...
    if (existingEvent) {
      console.log(`    🔍 Found duplicate: "${existingEvent.title}"`);
      return true;
    }

    return false;
  } catch (error) {
    // No duplicate found (expected for new events)
    return false;
  }
}
...
async function cleanupExistingDuplicates() {
  ...
  try {
    // Get all events grouped by external_id to find duplicates
    ...
    // Group events by external_id to find duplicates
    const eventGroups = {};
    allEvents.forEach((event) => {
      const key = event.external_id || `${event.title}|||${event.event_date}`;
      if (!eventGroups[key]) {
        eventGroups[key] = [];
      }
