// scripts/importEventsTest.js - Test version for sample university
const fs = require("fs");
...
// Test University ID - provided by user
const TEST_UNIVERSITY_ID = "6d7b9faa-d299-4d02-8959-840ba08c22b2";
...
function parseEventData(eventInfo, dateStr, sourceFile) {
  try {
    ...
    // Process location
    let location = isHomeGame ? "Test City, State" : null;
    let venue = null;
    ...
    if (isHomeGame) {
      homeCollegeId = TEST_UNIVERSITY_ID; // Test University is home
    } else {
      awayCollegeId = TEST_UNIVERSITY_ID; // Test University is away
    }
    ...
    const event = {
      ...
      home_team: isHomeGame ? "Test University" : opponent,
      away_team: isHomeGame ? opponent : "Test University",
      college_id: TEST_UNIVERSITY_ID,
...
function generateExternalId(sport, dateStr, opponent, isHomeGame) {
  ...
  return `${sport}_${cleanDate}_${gameType}_${cleanOpponent}_TEST`.toLowerCase();
}
...
async function main() {
  ...
  console.log(`🚀 Test University Event Import Script`);
  console.log(`=====================================`);

  if (!inputPath) {
    console.error(
      "Usage: node scripts/importEventsTest.js <directory-path>"
    );
    console.error(
      "Example: node scripts/importEventsTest.js ./calendar-data/test-university"
    );
    process.exit(1);
  }
  ...
  console.log(`🆔 College ID: ${TEST_UNIVERSITY_ID}\n`);
