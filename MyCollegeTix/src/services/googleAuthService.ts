...
class GoogleAuthService {
  ...
  constructor() {
    ...
    console.log(
      "⚠️  Make sure this EXACT URI is added to Supabase > Auth > URL Configuration > Redirect URLs"
    );
  }
  ...
  private async handleOAuthCallback(
    ...
    try {
      ...
      if (sessionError) {
        ...
        // Check if this is an email validation error from the database trigger
        if (
          sessionError.message.includes("limited to students") ||
          sessionError.message.includes("college email") ||
          sessionError.message.includes("Email domain not allowed")
        ) {
          return {
            error: new Error(
        ...
        return {
          error: new Error(`Session creation failed: ${sessionError.message}`),
        };
      }
      ...
      return {
        error: new Error(`Callback processing failed: ${error.message}`),
      };
    }
  }
