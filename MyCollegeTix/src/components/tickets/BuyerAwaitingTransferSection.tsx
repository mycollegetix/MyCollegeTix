// src/components/tickets/BuyerAwaitingTransferSection.tsx
// Section for buyer's purchases where payment is held but seller hasn't transferred yet
// Informational only — buyer just waits. Uses blue "pending" styling.
...
interface BuyerAwaitingTransferSectionProps {
  items: OrderItem[];
  expanded: boolean;
  onToggle: () => void;
}

export function BuyerAwaitingTransferSection({
  items,
  expanded,
  onToggle,
}: BuyerAwaitingTransferSectionProps) {
  ...
  return (
    <CollapsibleSection
      ...
      subtitle={`${items.length} ticket${items.length !== 1 ? "s" : ""} waiting for seller`}
      count={items.length}
    ...
    >
      <InfoBanner text="Payment has been captured. Waiting for the seller to transfer your tickets." />

      {items.map((item) => (
        <View key={`buyer-awaiting-${item.id}`} style={styles.card}>
          {/* Status Banner */}
...
export default BuyerAwaitingTransferSection;
